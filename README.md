В случае затруднений с развертыванием сервиса готов помочь и продемонстрировать на своей локальной машине всю работу сервиса
...
В нашем сервисе осуществлен парсер данных, который может перевести данные о полете в таблицу БД с заданными полями. При этом за небольшой промежуток времени

<img width="1919" height="1029" alt="image" src="https://github.com/user-attachments/assets/de4e8e78-a208-4157-a87b-da098fef3a45" />

При этом на скриншоте представлены распарсенные данные в количестве более 74 тыс. строк. Время обработки данных составило около 10 минут (точные подсчет времени не осуществлялся)
Данные были преобразованы из обычной записи в таблице excel с помощью модуля re на языке python (использовались регулярные выражения)
Также такие данные можно впоследующем использовать для прогнозирования и составления модели на основе алгоритмов машинного обучения.

Данный проект является пилотным и в нем реализованы не все функции. Оснавная цель проекта - демонстрация сил и возможностей команды, предоставление макета сервиса.
